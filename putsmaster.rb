puts "Mary had a little lamb"
#puts 'Mary has a little lamb'
puts "Its fleece was white as %s" %'snow'
#puts 'Its fleece was white as' and we added the string of snow
puts "And every where that Mary went"
#puts 'And every where that Mary went'
puts "." * 10 #what's that do
# forgot to put '*'
# we have 10 '.'
end1 = "C"
end2 = "h"
end3 = "e"
end4 = "e"
end5 = "s"
end6 = "e"
end7 = "b"
end8 = "u"
end9 = "r"
end10 = "g"
end11 = "e"
end12 = "r"

#notice how we are using print instead of puts here. 
#and see what happens.
print end1 + end2 + end3 + end4 + end5 + end6
print end7 + end8 + end9 + end10 +end11 + end12
#prints the full word cheeseburger without a space

#this juts is a polite use of terminal, try removing it.
puts 
puts end1 + end2 + end3 + end4 + end5 + end6
#puts the word cheese
puts end7 + end8 + end9 + end10 +end11 + end12
#puts the word burger
